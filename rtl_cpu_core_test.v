...
module cpu_core(
    ...
    always @(posedge cpu_clk or posedge cpu_rst) begin
        if (cpu_rst)
            ...
            discard_fetch <= !ifetch_valid;
        else if (discard_fetch && ifetch_valid)
            discard_fetch <= 1'b0;
    end
    ...
    always @(posedge cpu_clk or posedge cpu_rst) begin
        if (cpu_rst) begin
            ...
            pc       <= pc_next;
        end else if (!stall_f && ifetch_valid && !discard_fetch) begin
            pc       <= pc_plus_4;
        end
    end
    ...
    always @(posedge cpu_clk or posedge cpu_rst) begin
        if (cpu_rst) begin
            ...
            if (fetch_pending) begin
                ...
                if_id_inst  <= 32'h0;
                if_id_valid <= 1'b0;
                if (ifetch_valid && !discard_fetch) begin
                    fetch_inst_r  <= ifetch_inst;
                    fetch_pc_r    <= pc;
                    fetch_pending <= 1'b1;
                end
            end
    ...
    always @(posedge cpu_clk or posedge cpu_rst) begin
        if (cpu_rst) begin
            ...
            id_ex_ram_rop   <= if_id_valid ? id_ram_rop : `RAM_EXT_N;
            id_ex_ram_wop   <= if_id_valid ? id_ram_wop : `RAM_WE_N;
            id_ex_is_mul    <= if_id_valid & id_is_mul;
            id_ex_is_div    <= if_id_valid & id_is_div;
            id_ex_rs1       <= id_rs1;
            id_ex_rs2       <= id_rs2;
            id_ex_rd        <= id_rd;
     ...
     **********************************************************************/
    // A store uses rs2 as data even though its ALU B input is the immediate.
    wire id_uses_rs1 = !id_alua_sel;
    wire id_uses_rs2 = !id_alub_sel | (id_ram_wop != `RAM_WE_N);
    wire load_use_ex = id_ex_valid
                     & if_id_valid
                     & (id_ex_ram_rop != `RAM_EXT_N)
                     & (((id_rs1 == id_ex_rd) & id_uses_rs1)
                      | ((id_rs2 == id_ex_rd) & id_uses_rs2))
                     & (id_ex_rd != 5'h0);
    // Keep the dependent instruction out of ID/EX for one more cycle while
    // the load result moves from EX/MEM into MEM/WB.
    wire load_use_mem = ex_mem_valid
                      & if_id_valid
                      & (ex_mem_ram_rop != `RAM_EXT_N)
                      & (((id_rs1 == ex_mem_rd) & id_uses_rs1)
                       | ((id_rs2 == ex_mem_rd) & id_uses_rs2))
                      & (ex_mem_rd != 5'h0);
    wire load_use_stall = load_use_ex | load_use_mem;

    // Control hazards (Phase 3)
    wire id_is_jal   = if_id_valid & (id_npc_op == `NPC_JMP);
    wire ex_is_jalr  = id_ex_valid & (id_ex_npc_op == `NPC_JALR);
    wire ex_br_taken /* verilator public */ = id_ex_valid
                                             & (id_ex_npc_op == `NPC_BRA) & ex_br;
    ...
    wire flush_f /* verilator public */ = !mem_stall
                                        & (id_is_jal | ex_br_taken | ex_is_jalr
                                           | mul_div_release);
    ...
    // Do not let ICache accept another request on the same edge that the
    // previous registered response advances PC, otherwise the old address is
    // submitted twice.
    assign ifetch_req  = !cpu_rst & !ifetch_valid & !fetch_pending;
    ...
    wire mul_div_stall = id_ex_valid & (id_ex_is_mul | id_ex_is_div)
    ...
    // Data accesses can take an arbitrary number of cycles through Cache/AXI.
    // Keep every younger pipeline stage stable until the MEM transaction finishes.
    wire stall_f  /* verilator public */ = mem_stall
                                          | (((load_use_stall & !flush_f) | mul_div_stall)
                                             & !flush_f);
    wire stall_d /* verilator public */ = mem_stall | mul_div_stall;
    wire stall_e = mem_stall | mul_div_stall | duplicate_mul;
    ...
    always @(posedge cpu_clk or posedge cpu_rst) begin
        if (cpu_rst) begin
            ...
            // The current memory instruction owns EX/MEM until its response.
            ...
            ex_mem_ram_rop  <= id_ex_valid ? id_ex_ram_rop : `RAM_EXT_N;
            ex_mem_ram_wop  <= id_ex_valid ? id_ex_ram_wop : `RAM_WE_N;
            ex_mem_rd       <= id_ex_rd;
            ex_mem_is_mul   <= id_ex_valid & id_ex_is_mul;
            ex_mem_is_div   <= id_ex_valid & id_ex_is_div;
        end
    end
    ...
    wire mem_is_load  = ex_mem_valid & (ex_mem_ram_rop != `RAM_EXT_N);
    wire mem_is_store = ex_mem_valid & (ex_mem_ram_wop != `RAM_WE_N);
    wire mem_access   = mem_is_load | mem_is_store;
    wire mem_done     = (mem_is_load & daccess_rvalid)
                      | (mem_is_store & daccess_wresp);
    assign mem_stall  = mem_access & !mem_done;

    // DCache latches a request in its IDLE state. Pulse the request once, then
    // wait for the corresponding response so the request is not submitted twice.
    reg mem_req_sent;
    always @(posedge cpu_clk or posedge cpu_rst) begin
        if (cpu_rst)
            mem_req_sent <= 1'b0;
        else if (!mem_access || mem_done)
            mem_req_sent <= 1'b0;
    ...
    always @(posedge cpu_clk or posedge cpu_rst) begin
        if (cpu_rst) begin
            ...
            // An older WB instruction commits once while MEM remains frozen.
            mem_wb_valid     <= 1'b0;
            mem_wb_rf_we     <= 1'b0;
        end else begin
            ...
            if (mem_is_load && daccess_rvalid)
                mem_wb_rdata <= daccess_rdata;
        end
    end
...
`ifdef RUN_TRACE
    ...
    // A store is visible to trace only once, when DCache reports completion.
    assign debug_mem_pc    = ex_mem_pc;
    assign debug_mem_we    = (mem_is_store & daccess_wresp) ? mem_da_wen : 4'h0;
    assign debug_mem_waddr = mem_da_addr;
    // Trace compares the architectural rs2 value, not the byte-lane-shifted
    // bus payload generated by MREQ for SB/SH.
    assign debug_mem_wdata = ex_mem_rd2;
`endif
