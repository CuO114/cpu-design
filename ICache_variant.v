...
`ifdef ENABLE_ICACHE /******** 不要修改此行代码 ********/
    ...
    // The generated cache RAM stores only the low tag bits. Keep the
    // remaining address bits alongside each line to prevent aliases.
    reg  [16:0] tag_hi [0:63];
    wire [16:0] tag_hi_from_cpu = inst_addr[31:15];
    ...
    wire [ 5:0]  cache_index    = (state == STAT1) ? cpu_raddr[9:4] : inst_addr[9:4];
    ...
    wire         hit            = (tag_from_cache == tag_from_cpu) && valid_bit &&
                                   (tag_hi[cache_index] === tag_hi_from_cpu);
    ...
    always @(posedge cpu_clk or posedge cpu_rst) begin
        if (cpu_rst) begin
            for (tag_i = 0; tag_i < 64; tag_i = tag_i + 1)
                tag_hi[tag_i] <= 17'h0;
        end else if (cache_we) begin
            tag_hi[cache_index] <= cpu_raddr[31:15];
        end
    end
